...
DetectionResult *TestScanner::detect_colors(Mat img, Mat ref, vector<ColorOutput> colors)
{
    ...
    if (!result)
    {
        ...
        return create_detection_result(colors, img.size().width, img.size().height, 0, image, 3);
    }
    ...
    if (!findBoxFromContours(contours, vertices, norm(src2, src3, NORM_L2)))
    {
        ...
        return create_detection_result(colors, img.size().width, img.size().height, 0, image, 1);
    }
    ...
    return create_detection_result(colors, img.size().width, img.size().height, 0, image, 0);
}
